...
function stripPara(s: string): string {
  return s.replace(/\n\n$/, '');
}
...
export function emit(node: AstNode, ctx: EmitContext = DEFAULT_CTX): string {
  ...
  switch (n.tag) {
    ...
    // Superscript / subscript (outside math)
    case 'sup':   return `$^{${ch()}}$`;
    ...
    // Inline math structures (used inside other math envs)
    ...
    case 'verbatim':   return env('verbatim',   ch(), '');
    case 'verbatim*':  return env('verbatim*',  ch(), '');
    case 'verb': {
      const del = a('delim', '|');
      return `\\verb${del}${ch()}${del}`;
    }
